...
if(substr({rs[0]['direccion']},82,83) <>' '){
	$line_ini = $line_ini + $line_salto;
	$pdf->SetXY($colu_ini, $line_ini);//Coordenadas X/Y
	$pdf->MultiCell(140, $font_atl, '-'.utf8_decode(substr({rs[0]['direccion']},82,150)), 0,'L');
//	$pdf->MultiCell(20,$font_atl, utf8_decode('PUTA PUTA: '), 0,'l'); 
}
$line_salto = 4;
$line_ini = $line_ini + $line_salto;
$pdf->SetXY($colu_ini, $line_ini);//Coordenadas X/Y
$pdf->MultiCell(60, $font_atl, utf8_decode('R.I.F.: '.{rs[0]['codigo_proveedor']}), 0,'L');
$line_ini = $line_ini + $line_salto;
$pdf->SetXY($colu_ini, $line_ini); //Coordenadas X/Y
$pdf->MultiCell(100, $font_atl, utf8_decode('Telefono: '.{rs[0]['telefono']}), 0,'L');
...
$sql_det ="SELECT
	cd.codigo_producto,
	cd.nombre_producto,
	cd.precio_unitario_bs,
	cd.tipo_impuesto,
	cd.tipo_unidad,
	cd.cantidad,
	cd.total_iva_bs,
	cd.subtotal_renglon_bs,
	cd.total_renglon_bs,
...
WHERE
	...
	AND ip.empresa = cd.empresa 
	AND ip.sucursal = cd.sucursal ";
