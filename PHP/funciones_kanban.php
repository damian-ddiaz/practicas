...
sc_select(my_data, "
SELECT codigo_departamentos, nombre_departamentos,color
FROM configuracion_departamentos 
WHERE empresa = '[usr_empresa]'", "conn_helpdesk");
if ({my_data} === false) {
...
while (!$my_data->EOF) {
 	$codigo_dep 				= $my_data->fields[0];
    $var_nombre_departamentos	= $my_data->fields[1];
    $color 						= $my_data->fields[2];
	
    // ID único basado en el código de departamento
    $id_unico = "elemento-".$codigo_dep; // Corrección aquí

   $contenido = array(
        "id" => $codigo_dep,
        "title" => $var_nombre_departamentos, //utf8_decode($_var_nombre)
	...
	<style>
		...
		.<?php echo $id_unico; ?>{
				background-color: <?php echo $color; ?>;  /* Color de fondo del encabezado */
		}
...
WHERE 
    empresa = '[usr_empresa]' AND departamento = '$codigo_dep'  
	...
	<script>
        ...
        dropEl: function(el, target, source, sibling){
          let conversationId = el.getAttribute('data-eid');//Obtener el ID de la conversación
			...
			fetch('../updateDepartamento/updateDepartamento.php', {
				method: 'POST',
				headers: {
		...
		function abrirDetalle(idResumen, nombre_cliente, numeroPerfil) {
		    var url = '../chat_detalle_kanban/chat_detalle_kanban.php?par_id_resume=' + encodeURIComponent(idResumen) +
		  ...
		  overlay.style.cssText = "position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); display: flex; justify-content: center; align-items: center;";
		    ...
		    cerrarButton.style.cssText = "position: relative; right: 70px; bottom: 270px; cursor: pointer; color: red; width: 40px; height: 40px; font-size: 18px; font-weight: bold; border-radius: 5px; transparent; border: none; background-color 0.3s;  border: 2px solid black;";
					
			cerrarButton.onmouseover = function() {
    			cerrarButton.style.backgroundColor = "rgba(255, 0, 0, 0.2)"; // Color de fondo al pasar el ratón
			};

			cerrarButton.onmouseout = function() {
    			cerrarButton.style.backgroundColor = "transparent"; // Restablecer color de fondo
			};
