...
WHERE empresa = '[usr_empresa]' Limit 1", "conn_example");
	...
	*/
    ...
    if ({my_item} === false) {
        ...
        while (!$my_item->EOF) {
			...
			sc_select(my_color,"SELECT codigo, color FROM configuracion_tipo_prioridad 
    WHERE  empresa = '[usr_empresa]' and codigo = '$var_tipo_priorodad'", "conn_example");
			$var_color		 	= $my_color->fields[1];
			
		
            $items[] = array(
                "id" => $id_resumen,
   			    "title" => '<span class="mensaje-cuadro">' . $var_cantidad . '</span>'. $nombre_cliente . 
				'<div class="caja-boton"><button class="boton-detalle" onclick="abrirDetalle(' . $id_resumen . ', \'' . $nombre_cliente . '\')">Detalle 
				</button>
...
<div id="container" class="trello"></div>
	...
	// alert("Este es un mensaje de parada 1.");
