'<?php
    ...
    $user_nivel = isset([usr_nivel]) ? [usr_nivel] : '';
    ...
    if (isset($_POST['user_to_save']) && $_POST['user_to_save'] != '') {
		...
		//** Nuevo **//
		sc_lookup(rs_codigo_nivel, "select codigo_nivel from seguridad_users 
		WHERE login = '$login_esc'");		
		
		$var_codigo_nivel = {rs_codigo_nivel[0][0]};
	
		if($var_codigo_nivel <= 8){
			sc_exec_sql("UPDATE seguridad_users 
			SET 
				url_portal_aliado 				= 'NO',
				url_portal_cliente 				= 'NO',
				url_portal_dispositos 			= 'NO',
				url_portal_intranet 			= 'NO',
				url_portal_rrhh 				= 'NO',
				url_portal_administrativo 		= 'NO',
				url_portal_atencion_comercial 	= 'NO',
				url_portal_contabilidad 		= 'NO',
				url_portal_helpdesk 			= 'NO',
				url_portal_mantenimiento 		= 'NO',
				url_portal_soporte 				= 'NO',
				url_portal_vendedor 			= 'NO',
				url_portal_proyectos 			= 'NO',
				url_portal_eventos 				= 'NO',
				url_portal_finanzas 			= 'NO',
				url_portal_tributario 			= 'NO',
				url_portal_gestionredes 		= 'NO',
				url_portal_tickets 				= 'NO',
				url_portal_inventario 			= 'NO',
				url_portal_puntodeventa 		= 'NO',
				url_portal_reportes 			= 'NO',
				url_portal_formularios 			= 'NO',
				url_portal_herramientas 		= 'NO',
				url_portal_reportes_matriz 		= 'NO',
				url_portal_apps_compartidas 	= 'NO',
				url_portal_configuracion 		= 'NO',
				url_portal_patrocinadores 		= 'NO',
				url_portal_teblero_gerencial	= 'NO',
				url_portal_eventos				= 'NO',
				url_portal_tributario			= 'NO',
				url_portal_tickets				= 'NO',
				url_portal_formularios			= 'NO',
                        ...
                        AND  sg.modulo = $id_modulo_maestro");
                            ...
                            */
                            ...
                            if($id_modulo_maestro == 23){ // APLICACONES COMPARTIDAS SI 
                                if (!empty({rs_assigned_grupo})) {
                                    sc_exec_sql("UPDATE seguridad_users SET url_portal_apps_compartidas = 'SI'
                                    WHERE login = '$login_esc'");
                                }
                            }			
                    }
		...
		sc_lookup(group_default, "select login, group_id from seguridad_users_groups 
		WHERE login = '$login_esc' and group_id = 2");
				
		if(!isset({group_default[0][0]})){	
    ...
    if ($user_login != '') {
		...
		<input type="hidden" name="last_module" id="last_module" value="'.(isset($_POST['last_module']) ? $_POST['last_module'] : '').'">
	...
	function selectModule(id, el){
		...
		// Nueva línea: guarda el ID en el input oculto
		document.getElementById("last_module").value = id;
	}
	...
	document.addEventListener("DOMContentLoaded", function(){
		// Obtenemos el ID guardado del input oculto
		const lastId = document.getElementById("last_module").value;

		if (lastId) {
			// CORRECCIÓN: Usamos comillas simples para PHP y escapamos las dobles del selector
			const savedModule = document.querySelector(".module-card[onclick*=\"" + lastId + "\"]");
			if (savedModule) {
				savedModule.click();
				// Esto hace que el carrusel se mueva automáticamente hasta el módulo guardado
				savedModule.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" });
			}
		} else {
